package
{
   ...
   public class HabboMain extends Sprite implements IHabboMain
   {
      ...
      private var var_690:ICore;
      
      private var var_1505:ICoreConfigStatusListener;
      ...
      private function initCore(param1:String, param2:uint) : void
      {
         ...
         var_690 = Core.instantiate(this,param2);
         initBakedInComponents();
         var_690.events.addEventListener(Component.COMPONENT_EVENT_ERROR,onCoreError);
         var_690.loadExternalConfigDocument(new URLRequest(param1),var_1505);
      }
      ...
      protected function onStageInit(param1:Event = null) : void
      {
         ...
         initCore("config_habbo.xml",Core.const_829);
      }
      ...
      public function initializeCore() : void
      {
         ...
         {
            Core.crash("Failed to initialize the core: " + error.message,CoreComponent.const_1212,error);
         }
      }
      ...
      private function prepareAssetLibrary(param1:Class) : void
      {
         ...
         var_690.prepareAssetLibrary(manifest,resource);
      }
      ...
      private function prepareBakedInComponent(param1:Class) : void
      {
         ...
         var bytes:ByteArray = new xmlClass() as ByteArray;
         var manifest:XML = new XML(bytes.readUTFBytes(bytes.length));
         var_690.prepareComponent(manifest,resource);
      }
