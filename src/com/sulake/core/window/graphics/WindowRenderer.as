package com.sulake.core.window.graphics
{
   ...
   public class WindowRenderer implements IWindowRenderer
   {
      ...
      public function WindowRenderer(param1:ISkinContainer)
      {
         ...
         var_538 = new Array();
         var_1421 = new Point();
         var_1422 = new Rectangle();
         var_290 = new Rectangle();
         var_291 = new Dictionary(true);
         var_595 = param1;
      ...
      {
         var _loc2_:WindowRendererItem = var_291[param1] as WindowRendererItem;
      ...
      public function render() : void
      {
         ...
         while(_loc4_ < _loc2_)
         {
            _loc1_ = var_538.pop() as WindowController;
            if(!_loc1_.disposed)
            {
               _loc3_ = getWindowRendererItem(_loc1_);
               var_290.x = _loc3_.dirty.x;
               var_290.y = _loc3_.dirty.y;
               var_290.width = _loc3_.dirty.width;
               var_290.height = _loc3_.dirty.height;
               renderWindowBranch(_loc1_,_loc3_,var_290);
            }
            _loc4_++;
         }
      }
      ...
      {
         param1.removeEventListener(WindowDisposeEvent.const_490,windowDisposedCallback);
         var _loc2_:WindowRendererItem = var_291[param1] as WindowRendererItem;
         if(_loc2_ != null)
         {
            _loc2_.dispose();
            delete var_291[param1];
         }
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            _disposed = true;
            var_538 = null;
            for(_loc1_ in var_291)
            {
               delete var_291[_loc1_];
            }
            var_291 = null;
      ...
      private function renderWindowBranch(param1:WindowController, param2:WindowRendererItem, param3:Rectangle) : void
      {
         ...
         if(param1.visible)
         {
            if(getDrawLocationAndClipRegion(param1,param3,var_1421,var_1422))
            {
               if(param2.render(param1,var_1421,var_1422))
               {
      ...
      public function registerRenderable(param1:IWindow) : void
      {
         ...
         if(_loc2_ == null)
         {
            _loc2_ = new WindowRendererItem(this,_drawBufferAllocator,var_595);
            var_291[param1] = _loc2_;
            param1.addEventListener(WindowDisposeEvent.const_490,windowDisposedCallback);
         }
      }
      ...
      public function invalidate(param1:IWindowContext, param2:Rectangle) : void
      {
         ...
         while(_loc4_-- > 0)
         {
            ...
            addToRenderQueue(_loc5_,_loc5_.rectangle,WindowRedrawFlag.const_68);
         }
      }
      ...
      public function addToRenderQueue(param1:IWindow, param2:Rectangle, param3:uint) : void
      {
         ...
         if(getWindowRendererItem(param1).invalidate(param1,param2,param3))
         {
            ...
            var_290.x = param2.x - param1.x;
            var_290.y = param2.y - param1.y;
            var_290.width = param2.width;
            var_290.height = param2.height;
            if(param1.testParamFlag(WindowParam.const_32))
            {
               while(param1.testParamFlag(WindowParam.const_32))
               {
                  ...
                  var_290.offset(param1.x,param1.y);
                  param1 = _loc4_;
               }
            }
            getWindowRendererItem(param1).invalidate(param1,var_290,WindowRedrawFlag.const_238);
            if(var_538.indexOf(param1) == -1)
            {
               var_538.push(param1);
            }
         }
