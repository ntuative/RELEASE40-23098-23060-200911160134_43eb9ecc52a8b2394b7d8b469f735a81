package com.sulake.core.window.utils
{
   ...
   public class WindowParser implements IWindowParser
   {
      ...
      public static const const_1436:String = "_INCLUDE";
      
      public static const const_783:String = "_EXCLUDE";
      
      public static const const_1534:String = "_TEMP";
      
      private static const const_1085:RegExp = /^(\s|\n|\r|\t|\v)*/m;
      ...
      protected var var_859:Dictionary;
      
      protected var var_1278:Map;
      
      protected var _context:IWindowContext;
      
      protected var var_1279:Dictionary;
      
      protected var var_1032:Dictionary;
      
      public function WindowParser(param1:IWindowContext)
      {
         super();
         _context = param1;
         var_1032 = new Dictionary();
         var_1031 = new Dictionary();
         TypeCodeTable.fillTables(var_1032,var_1031);
         var_859 = new Dictionary();
         var_1279 = new Dictionary();
         ParamCodeTable.fillTables(var_859,var_1279);
         var_1278 = new Map();
      }
      
      private static function trimWhiteSpace(param1:String) : String
      {
         param1 = param1.replace(const_1086,"");
         return param1.replace(const_1085,"");
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            for(_loc1_ in var_1032)
            {
               delete var_1032[_loc1_];
            }
            for(_loc1_ in var_1031)
            {
               delete var_1031[_loc1_];
            }
            for(_loc1_ in var_859)
            {
               delete var_859[_loc1_];
            }
            for(_loc1_ in var_1279)
            {
               delete var_1279[_loc1_];
            }
            var_1278.dispose();
            var_1278 = null;
            _context = null;
      ...
      private function parseSingleWindowEntity(param1:XML, param2:WindowController, param3:Map) : IWindow
      {
         ...
         background = parseAttribute(xml,"background",variables,background.toString()) == "true";
         treshold = uint(parseAttribute(xml,"treshold",variables,treshold.toString()));
         visible = parseAttribute(xml,"visible",variables,visible.toString()) == "true";
         clipping = parseAttribute(xml,"clipping",variables,clipping.toString()) == "true";
         caption = unescape(parseAttribute(xml,"caption",variables,caption));
         color = parseAttribute(xml,"color",variables,color);
         blend = Number(parseAttribute(xml,"blend",variables,blend.toString()));
         ...
         window = _context.create(name,caption,type,style,params,rect,null,parent is IIterable ? null : parent,id,parseProperties(xml.child("variables")[0]),tags) as WindowController;
         ...
         if(window.alphaTreshold != treshold)
         {
            window.alphaTreshold = treshold;
         }
            ...
            if(parent != null)
            {
               if(parent is IIterable)
               {
                  if(window.x != rect.x || window.y != rect.y || window.width != rect.width || window.height != rect.height)
                  {
                     ...
                     if((params & 0) == WindowParam.const_152)
                     {
                        window.y = rect.y;
                     }
                  }
         ...
         if(_loc7_ is IIterable)
         {
            ...
            if(_loc9_ > 0)
            {
               ...
               while(_loc12_ < _loc9_)
               {
                  ...
                  if(_loc10_.tags.indexOf(WindowParser.const_783) == -1)
                  {
