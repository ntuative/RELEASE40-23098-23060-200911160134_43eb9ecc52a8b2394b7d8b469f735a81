package com.sulake.core.window.utils
{
   ...
   public class WindowMouseOperator implements IDisposable
   {
      ...
      protected var var_485:Point;
      
      protected var var_122:DisplayObject;
      
      private var _disposed:Boolean = false;
      
      protected var var_1026:uint;
      ...
      protected function handler(param1:Event) : void
      {
         ...
         if(_working)
         {
            if(param1.type == Event.ENTER_FRAME)
            {
               ...
               {
                  operate(var_122.mouseX,var_122.mouseY);
      ...
      public function operate(param1:int, param2:int) : void
      {
         _mouse.x = param1;
         _mouse.y = param2;
         getMousePositionRelativeTo(_window,_mouse,var_485);
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         if(_working)
         {
            if(_window == param1)
            {
               var_122.removeEventListener(MouseEvent.MOUSE_DOWN,handler,false);
               var_122.removeEventListener(MouseEvent.MOUSE_UP,handler,false);
               var_122.removeEventListener(Event.ENTER_FRAME,handler);
               if(true)
               {
                  _window.removeEventListener(WindowEvent.const_392,clientWindowDestroyed);
               }
               _window = null;
               _working = false;
            }
      ...
      public function begin(param1:IWindow, param2:uint = 0) : IWindow
      {
         ...
         {
            var_122.addEventListener(MouseEvent.MOUSE_DOWN,handler,false);
            var_122.addEventListener(MouseEvent.MOUSE_UP,handler,false);
            var_122.addEventListener(Event.ENTER_FRAME,handler);
            _mouse.x = var_122.mouseX;
            _mouse.y = var_122.mouseY;
            _window = WindowController(param1);
            getMousePositionRelativeTo(param1,_mouse,_offset);
            _window.addEventListener(WindowEvent.const_392,clientWindowDestroyed);
            _working = true;
         }
         return _loc3_;
      }
