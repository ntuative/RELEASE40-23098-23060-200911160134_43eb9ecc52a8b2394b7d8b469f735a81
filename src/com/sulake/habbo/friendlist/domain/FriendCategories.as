package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private var var_355:IFriendCategoriesDeps;
      
      private var var_1111:Dictionary;
      
      public function FriendCategories(param1:IFriendCategoriesDeps)
      {
         var_323 = new Array();
         var_1111 = new Dictionary();
         super();
         var_355 = param1;
      }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         var _loc5_:* = null;
         flushReceivedStatus();
         findCategory(FriendCategory.const_425).received = true;
         findCategory(FriendCategory.const_980).received = true;
      ...
      private function removeFriend(param1:int, param2:Boolean) : Boolean
      {
         ...
         if(param2)
         {
            var_1111[param1] = null;
         }
         var _loc3_:Boolean = false;
         for each(_loc4_ in var_323)
         {
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc2_.updatedFriends)
         {
            ...
            var_355.messenger.setFollowingAllowed(_loc4_.id,_loc4_.followingAllowed && _loc4_.online);
            _loc6_ = isFriendOnline(_loc4_.id);
            if(_loc6_ && !_loc4_.online)
            {
               var_355.messenger.setOnlineStatus(_loc4_.id,_loc4_.online);
               var_355.notifications.addOfflineNotification(_loc4_.name,_loc4_.realName);
            }
            if(!_loc6_ && _loc4_.online)
            {
               var_355.messenger.setOnlineStatus(_loc4_.id,_loc4_.online);
               var_355.view.setNewMessageArrived();
               var_355.notifications.addOnlineNotification(_loc4_.name,_loc4_.figure,_loc4_.realName);
            }
      ...
      {
         var _loc2_:int = !!param1.online ? int(param1.categoryId) : int(FriendCategory.const_425);
         var _loc3_:FriendCategory = findCategory(_loc2_);
         if(_loc3_ != null)
         {
            _loc3_.addFriend(param1);
            var_1111[param1.id] = param1;
            return _loc3_;
         }
