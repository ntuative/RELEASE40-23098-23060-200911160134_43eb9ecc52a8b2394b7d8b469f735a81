   ...
   public class GroupDetailsView
   {
      ...
      private function openExternalLink(param1:String) : void
      {
         if(param1 != "")
         {
            ...
            navigateToURL(new URLRequest(param1),"_empty");
         }
      }
      ...
      private function showGroupInfoDialog(param1:int) : void
      {
         ...
         if(var_160 == null)
         {
            ...
            if(_loc3_ != null)
            {
               var_160 = _sessionManager.windowManager.createWindow("habbo_group_details_window","${group.window.title}",HabboWindowType.const_316,HabboWindowStyle.const_38,0 | 0,new Rectangle(10,10,300,220),null) as IWindowContainer;
               var_160.buildFromXML(_loc3_.content as XML) as IWindowContainer;
               _loc4_ = var_160.findChildByTag("close");
               if(_loc4_ != null)
               {
               ...
               {
                  _loc7_ = var_160.findChildByName(_loc6_);
                  if(_loc7_ != null)
                  {
                     _loc7_.alphaTreshold = 0;
         ...
         if(var_160 != null)
         {
            var_160.visible = true;
            var_160.center();
            var_160.findChildByName("group_name").caption = _loc2_.name;
            var_160.findChildByName("group_description").caption = _loc2_.description;
            if(_loc2_.roomId < 0)
            {
               var_160.findChildByName("group_room_link").visible = false;
            }
            else
            {
               var_160.findChildByName("group_room_link").visible = true;
            }
            _sessionManager.windowManager.registerLocalizationParameter("group.room.link","room_name",_loc2_.roomName);
            _loc8_ = var_160.findChildByName("group_logo") as IBitmapWrapperWindow;
            if(_loc8_ != null)
            {
      ...
      private function onGroupDetailsEvents(param1:Event) : void
      {
         ...
         var _loc3_:HabboGroupDetails = _sessionManager.getGroupDetails(var_2034);
         switch(_loc2_.name)
         {
