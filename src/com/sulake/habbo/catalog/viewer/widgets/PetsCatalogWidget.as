package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         var petColor:IPetColor = var_282[var_361] as IPetColor;
         var color:uint = petColor.rgb;
         var p:String = _name + String.fromCharCode(10) + addZeroPadding(String(var_1354),3) + String.fromCharCode(10) + addZeroPadding(color.toString(16).toUpperCase(),6);
         return p;
      }
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_909.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_224,onTextWindowEvent);
         var_909 = new Map();
         ...
         var_723 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_910 = 0;
         var_282 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_723);
               if(_loc11_)
               {
                  var_910 = _loc11_.breeds.length;
                  var_282 = _loc11_.colors;
                  if(var_282 == null)
                  {
                     var_282 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_910)
         {
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_723,_loc6_),getRaceLocalizationKey(var_723,_loc6_)));
            _loc6_++;
         }
         var_909.add(var_723,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_975,onApproveNameResult);
         _loc5_ = var_909.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_282)
         {
      ...
      {
         var _loc6_:* = null;
         var _loc1_:Offer = var_909.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_361 >= 0 && var_361 < var_282.length)
         {
            _loc6_ = var_282[var_361] as IPetColor;
            if(_loc6_ != null)
            {
               _loc2_ = uint(_loc6_.rgb);
            }
         }
         var _loc3_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_723,var_1354,_loc2_,AvatarScaleType.const_50);
         if(_loc3_ == null)
         {
            return;
         }
         _loc3_.initActionAppends();
         _loc3_.appendAction(AvatarAction.const_311,AvatarAction.const_565);
