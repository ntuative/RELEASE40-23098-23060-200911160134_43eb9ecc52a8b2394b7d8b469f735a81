package com.sulake.habbo.room.object.visualization.room.rasterizer.basic
{
   ...
   public class FloorRasterizer extends PlaneRasterizer
   {
      ...
      private function parseFloors(param1:XML) : void
      {
         ...
         while(_loc3_ < _loc2_.length())
         {
            ...
            if(XMLValidator.checkRequiredAttributes(_loc4_,["id"]))
            {
               ...
               if(var_318.getValue(_loc5_) == null)
               {
                  var_318.add(_loc5_,_loc7_);
               }
               else
               {
                  _loc7_.dispose();
      ...
      {
         var _loc13_:FloorPlane = var_318.getValue(param2);
         if(_loc13_ == null)
         {
            _loc13_ = var_318.getValue(const_724);
         }
         if(_loc13_ == null)
         {
            return null;
      ...
      override public function initializeAssetCollection(param1:IGraphicAssetCollection) : void
      {
         ...
         var _loc2_:XMLList = data.floors;
         if(_loc2_.length() > 0)
         {
            parseFloors(_loc2_[0]);
         }
      }
      ...
      override public function dispose() : void
      {
         ...
         {
            _loc1_ = 0;
            while(_loc1_ < var_318.length)
            {
               _loc2_ = var_318.getWithIndex(_loc1_) as FloorPlane;
               if(_loc2_ != null)
               {
                  _loc2_.dispose();
               }
